...
def calculatePageRank(links,pagerank):
...
if __name__ == "__main__":
	...
	seperateLinks= links.map(lambda x: parseText(x)) 
	...
	for x in range(0, 10):
		rank = links.join(pagerank).flatMap(lambda x : calculatePageRank(x[1][0],x[1][1]))
